...
class PagosController {
  static async crearPreferenciaPago(req, res) {
    try {
      ...
      const preferenceData = {
        body: {
          items: [{
            title: `Servicio ${contratacion.servicio_descripcion.substring(0, 50)}`, // Limita longitud
          ...
          payment_methods: {
            ...
            installments: 1 // Evita cuotas en sandbox
          },
          external_reference: id_contratacion.toString(),
          notification_url: `${process.env.BACKEND_URL}/api/pagos/webhook`,
          back_urls: {
            success: `${process.env.FRONTEND_URL}/pago-exitoso?id=${id_contratacion}`,
            failure: `${process.env.FRONTEND_URL}/pago-fallido?id=${id_contratacion}`,
          },
  ...
  static async manejarWebhook(req, res) {
    try {
      ...
      res.status(400).json({ error: 'Webhook fallido' });
    }
  }
}
